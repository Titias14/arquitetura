...
// Função para calcular a raiz quadrada inversa usando o método Quake III
float quake_inv_sqrt(float number) {
    long i;
    float x2, y;
    ...
    x2 = number * 0.5F;
    y = number;
    i = *(long*)&y;                      // Converte o float para um inteiro
    i = 0x5f3759df - (i >> 1);           // Estimativa inicial mágica
    y = *(float*)&i;                      // Converte de volta para float

    // Iteração de Newton-Raphson para melhorar a precisão
    y = y * (threehalfs - (x2 * y * y));  // Primeira iteração (normalmente suficiente)

    return y;
}

// Função otimizada para normalizar um vetor de características usando o método Quake III
void normalize_feature_vector(float* features, int length) {
    ...
    float inv_sqrt = quake_inv_sqrt(sum);
